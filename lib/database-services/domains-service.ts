...
import type { Domain, User } from "@/lib/types" // Assuming you have a types file
...
export const domainsService = {
  ...
  async createDomain(
    ...
    const dataToInsert: Partial<Database["public"]["Tables"]["domains"]["Insert"]> = {
      ...domainData,
    ...
    const { data, error } = await supabase.from("domains").insert(dataToInsert).select().single()
  ...
  async updateDomain(
    ...
    const dataToUpdate: Partial<Database["public"]["Tables"]["domains"]["Update"]> = { ...domainData }

    if (domainData.status === "approved" && currentDomain.status !== "approved" && adminUserId) {
      dataToUpdate.approved_by = adminUserId
      dataToUpdate.approved_at = new Date().toISOString()
    } else if (domainData.status !== "approved" && currentDomain.status === "approved") {
      dataToUpdate.approved_by = null
      dataToUpdate.approved_at = null
    }

    const { data, error } = await supabase.from("domains").update(dataToUpdate).eq("id", id).select().single()
  ...
  async getDomainStats(): Promise<{
    ...
    try {
      ...
      console.error("Failed to fetch total domains count:", err.message || err, err)
    }
    ...
    try {
      ...
      console.error("Failed to fetch pending domains count:", err.message || err, err)
    }
    ...
    try {
      ...
      console.error("Failed to fetch approved domains count:", err.message || err, err)
    }
    ...
    try {
      ...
      console.error("Failed to fetch rejected domains count:", err.message || err, err)
    }
    ...
    try {
      ...
      console.error("Failed to fetch sold domains count:", err.message || err, err)
    }

    try {
      const { data: soldDomainsData, error } = await supabase.from("domains").select("price").eq("status", "sold")
      if (error) {
        console.error("Supabase error fetching revenue data:", error)
        throw error
      }
      totalRevenue = soldDomainsData?.reduce((sum, d) => sum + (d.price || 0), 0) || 0
    } catch (err: any) {
