...
export default function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error, user } = await signUp(email, password, username.trim(), role)
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#0d0d0f] p-4">
        <Card className="w-full max-w-md bg-[#1a1a1a] border-[#2a2a3a]">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl font-mono text-[#00ff9d]">Registration Successful</CardTitle>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#0d0d0f] p-4">
      <Card className="w-full max-w-md bg-[#1a1a1a] border-[#2a2a3a]">
        <CardHeader className="text-center">
          <CardTitle className="text-2xl font-mono text-[#00ff9d]">Request Access</CardTitle>
          <CardDescription className="text-gray-400">Create your TRXSecurity account</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="space-y-2">
              ...
              <Select value={role} onValueChange={(value: UserRole) => setRole(value)}>
                ...
                <SelectContent className="bg-[#2a2a3a] border-[#3a3a4a]">
                  <SelectItem value="client" className="text-white hover:bg-[#3a3a4a] font-mono">
                    🛒 Client - Browse and purchase domains
                  </SelectItem>
                  <SelectItem value="seller" className="text-white hover:bg-[#3a3a4a] font-mono">
          ...
          <div className="mt-6 text-center">
            <div className="text-sm text-gray-400">
              Already have access?{" "}
              <Link href="/auth/login" className="text-[#00ff9d] hover:underline">
                Sign in
              </Link>
            </div>
          </div>
