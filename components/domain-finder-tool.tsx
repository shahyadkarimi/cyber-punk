...
import { Search, Download, Globe, Cog, AlertCircle, X, Server, Copy, Check, Pause, Play } from "lucide-react"
...
export default function DomainFinderTool() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (activeTab === "reverse") {
        // Find domains by IP (reverse lookup)
        const domains = await findDomainsByIp(domain)
        result = domains.map((domainName) => ({
          domain: domainName,
          type: "reverse",
          cms: null,
          cmsConfidence: null,
      ...
      if (err.captchaRequired && err.captcha) {
        ...
        setError("Rate limit exceeded. Please complete the CAPTCHA to continue.")
      } else {
  ...
  useEffect(() => {
    ...
    const processCmsDetectionQueue = async () => {
      if (!cmsDetectionInProgress || cmsDetectionPaused || !results || cmsDetectionQueue.length === 0) {
        return
      }
      ...
      try {
        ...
        await Promise.all(
          batchIndices.map(async (index) => {
            ...
            try {
              ...
              if (isMounted) {
                ...
                setResults((prevResults) => {
                  ...
                  newResults[index] = {
                    ...
                    vulnerabilities: cmsInfo.vulnerabilities || [],
                    cmsDetected: true,
                    cmsDetectionFailed: cmsInfo.cms === "Error Checking",
                  }
              ...
              console.error(`Failed to detect CMS for domain at index ${index}:`, err)
        ...
        // Update progress
        if (isMounted) {
          const progress = ((cmsDetectionTotal - remainingQueue.length) / cmsDetectionTotal) * 100
    ...
    if (cmsDetectionInProgress && !cmsDetectionPaused && cmsDetectionQueue.length > 0) {
  ...
  }, [cmsDetectionInProgress, cmsDetectionPaused, cmsDetectionQueue, results, cmsDetectionTotal])
  ...
  return (
    <div className="space-y-6">
      <Tabs defaultValue="reverse" value={activeTab} onValueChange={handleTabChange} className="w-full">
        <TabsList className="grid grid-cols-2 mb-4 bg-[#1a1a1a] border border-[#2a2a3a]">
          <TabsTrigger value="reverse" className="data-[state=active]:bg-[#2a2a3a] data-[state=active]:text-[#00ff9d]">
        ...
        <TabsContent value="reverse" className="mt-0">
          <div className="p-4 border border-[#2a2a3a] bg-[#1a1a1a]/50 rounded-md mb-4">
            <p className="text-sm text-gray-400">
              Enter a domain or IP address to find all domains hosted on the same server.
            </p>
          </div>
        </TabsContent>
        ...
        <TabsContent value="subdomain" className="mt-0">
          <div className="p-4 border border-[#2a2a3a] bg-[#1a1a1a]/50 rounded-md mb-4">
            <p className="text-sm text-gray-400">
              Enter a domain to discover all its subdomains using various techniques including SSL certificate data.
            </p>
          </div>
        </TabsContent>
      </Tabs>
      ...
      {captchaRequired && captchaData ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="detectCMS" className="text-sm font-mono text-gray-300 cursor-pointer">
              Detect CMS and vulnerabilities (will be processed gradually after initial results)
            </Label>
          </div>
        </form>
      )}
      ...
      {cmsDetectionInProgress && (
        <div className="border border-[#2a2a3a] bg-[#1a1a1a]/50 rounded-md p-4 space-y-3">
          <div className="flex justify-between items-center">
            <div>
              <h3 className="text-sm font-mono text-gray-300">CMS Detection in Progress</h3>
              <p className="text-xs text-gray-400">
                Detecting {cmsDetectionCurrent} of {cmsDetectionTotal} domains ({Math.round(cmsDetectionProgress)}%)
              </p>
            </div>
            <Button
              variant="outline"
            ...
            {results.map((item, index) => (
              ...
              >
                <div className="bg-[#2a2a3a]/50 px-4 py-3 flex justify-between items-center">
                  <h3 className="font-mono text-lg text-[#00ff9d]">{item.domain}</h3>
                ...
                {item.cmsDetected && !item.cmsDetectionFailed && item.cms && (
                  <div className="p-4 space-y-3 border-t border-[#2a2a3a]">
                    <div className="flex justify-between items-center">
                      <div>
                        <h4 className="text-sm font-mono text-gray-400 mb-1">CMS</h4>
                    ...
                    {item.vulnerabilities && item.vulnerabilities.length > 0 && (
                      <div>
                        <h4 className="text-sm font-mono text-gray-400 mb-2">Known Vulnerabilities</h4>
                        <div className="space-y-2">
                          {item.vulnerabilities.map((vuln: string, i: number) => (
                            <div
                              key={i}
      ...
      {results && results.length === 0 && (
        <Alert className="bg-yellow-900/20 border-yellow-900/30 text-yellow-400">
          ...
          <AlertDescription>
            No {activeTab === "reverse" ? "domains" : "subdomains"} were found for the provided input.
          </AlertDescription>
        </Alert>
      )}
      ...
      {!results && !loading && !captchaRequired && (
        <div className="border border-[#2a2a3a] bg-[#1a1a1a]/50 rounded-md p-8 text-center backdrop-blur-sm">
          ...
          <h3 className="text-xl font-mono mb-2 text-gray-300">Enter a domain to start scanning</h3>
      ...
      <div className="border border-[#2a2a3a] bg-[#1a1a1a]/50 rounded-md p-4 text-sm text-gray-400 font-mono backdrop-blur-sm">
        <p className="mb-2">
          <span className="text-[#00ff9d]">Note:</span> This tool is for educational purposes and security research
          only.
        </p>
        <p>
          Always ensure you have permission to scan the target domain. Unauthorized scanning may violate laws and
          regulations.
        </p>
      </div>
    </div>
