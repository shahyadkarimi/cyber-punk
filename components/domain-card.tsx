...
export function DomainCard({ domain, viewMode = "grid" }: DomainCardProps) {
  ...
  const getCategoryColor = (category: string) => {
    const colors = {
      Technology: "bg-blue-100 text-blue-800 border-blue-200 dark:bg-blue-900 dark:text-blue-200 dark:border-blue-700",
      Education:
      ...
      Health: "bg-red-100 text-red-800 border-red-200 dark:bg-red-900 dark:text-red-200 dark:border-red-700",
      Finance:
            ...
            <div className="flex-1">
              <div className="flex items-center gap-4 mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100">{domain.domain}</h3>
                </div>
                {domain.category && <Badge className={getCategoryColor(domain.category)}>{domain.category}</Badge>}
              </div>
              ...
              <div className="flex items-center gap-6 text-sm">
                {domain.da_score && (
                  <div className="flex items-center gap-2 px-3 py-1 bg-red-50 dark:bg-red-900/20 rounded-full">
                    ...
                    <span className="text-slate-600 dark:text-slate-400">DA:</span>
                    <span className="text-red-600 dark:text-red-400 font-semibold">{domain.da_score}</span>
                  </div>
                )}
                {domain.pa_score && (
                  <div className="flex items-center gap-2 px-3 py-1 bg-purple-50 dark:bg-purple-900/20 rounded-full">
                    <TrendingUp className="h-4 w-4 text-purple-600 dark:text-purple-400" />
                    <span className="text-slate-600 dark:text-slate-400">PA:</span>
                    <span className="text-purple-600 dark:text-purple-400 font-semibold">{domain.pa_score}</span>
                  </div>
                )}
              </div>
            </div>
  ...
  return (
    <>
      <Card className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:shadow-lg transition-all duration-300 group">
        <CardContent className="p-6">
          ...
          <p className="text-slate-600 dark:text-slate-400 mb-6 line-clamp-3 min-h-[4.5rem]">
            {domain.description || "Premium domain with high SEO value and excellent potential for business growth."}
          </p>
            ...
            <div className="flex items-center justify-between">
              {domain.da_score && (
                <div className="flex items-center gap-2 px-3 py-2 bg-red-50 dark:bg-red-900/20 rounded-lg">
                  <TrendingUp className="h-4 w-4 text-red-600 dark:text-red-400" />
                  <span className="text-slate-600 dark:text-slate-400 text-sm">DA:</span>
                  <span className="text-red-600 dark:text-red-400 font-semibold">{domain.da_score}</span>
                </div>
              )}
              {domain.pa_score && (
                <div className="flex items-center gap-2 px-3 py-2 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                  <TrendingUp className="h-4 w-4 text-purple-600 dark:text-purple-400" />
                  <span className="text-slate-600 dark:text-slate-400 text-sm">PA:</span>
                  <span className="text-purple-600 dark:text-purple-400 font-semibold">{domain.pa_score}</span>
                </div>
              )}
            </div>
          ...
          <div className="text-center mb-6">
            ...
            <div className="flex items-center justify-center gap-1 mt-1">
              ...
              <span className="text-sm text-slate-500 dark:text-slate-400">Premium Quality</span>
            </div>
          </div>
        </CardContent>
      ...
      {showPurchaseModal && (
        <PurchaseModal domain={domain} isOpen={showPurchaseModal} onClose={() => setShowPurchaseModal(false)} />
      )}
    </>
  )
}
