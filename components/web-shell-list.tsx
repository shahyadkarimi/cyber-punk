...
export default function WebShellList() {
  ...
  const fetchShells = async () => {
    ...
    setError(null)
    try {
      const data = await ShellsService.getActiveShells()
      setShells(data)
    ...
    if (query) {
      result = result.filter(
        (shell) =>
          ...
          (shell.tags && shell.tags.some((tag) => tag.toLowerCase().includes(query.toLowerCase()))),
      )
    }

    // Apply language filter
    if (filter !== "all") {
      result = result.filter((shell) => shell.language.toLowerCase() === filter.toLowerCase())
    }
  ...
  return (
    <section>
      ...
      <SearchBar onSearch={handleSearch} onFilterChange={handleFilterChange} onRefresh={handleRefresh} />
